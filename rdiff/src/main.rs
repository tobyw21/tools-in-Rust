// starter code is from Standford University cs110L
...
fn lcs(seq1: &Vec<String>, seq2: &Vec<String>) -> Grid {
    ...
    for i in 0..m + 1 {
        match grid.set(i, 0, 0) {
            Ok(()) => ignore(),
            Err(errmsg) => eprintln!("{}", errmsg),
        }
    }

    for j in 0..n + 1 {
        match grid.set(0, j, 0) {
            Ok(()) => ignore(),
            Err(errmsg) => eprintln!("{}", errmsg),
        }
    }
    ...
    for i in 0..m {
        for j in 0..n {
            if seq1[i] == seq2[j] {
                let getval = grid.get(i, j).unwrap() + 1;
                match grid.set(i + 1, j + 1, getval) {
                    Ok(()) => ignore(),
                    Err(errmsg) => eprintln!("{}", errmsg),
                }

            } else {
                let val1 =  grid.get(i + 1, j).unwrap();
                let val2: usize = grid.get(i, j + 1).unwrap();
                let maxval = max(val1, val2);

                match grid.set(i + 1, j + 1, maxval) {
                    Ok(()) => ignore(),
                    Err(errmsg) => eprintln!("{}", errmsg),
                }
            }
...
fn print_diff(lcs_table: &Grid, lines1: &Vec<String>, lines2: &Vec<String>, i: usize, j: usize) {
    ...
    // which would consume a lot of stack space
    // in order to make it efficient
    // change the function to iterative function
...
fn main() {
    ...
    let filelines1: Vec<String> = read_file_lines(filename1).expect("read file error!");
    let filelines2: Vec<String> = read_file_lines(filename2).expect("read file error!");

    let grid: Grid = lcs(&filelines1, &filelines2);


    print_diff(&grid, &filelines1, &filelines2, filelines1.len(), filelines2.len());
}
