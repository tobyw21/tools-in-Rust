...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        ...
        let linkbuf = fs::read_link(path).ok()?;

        let proc_fd_name = OpenFile::path_to_name(linkbuf.to_str()?);

        let file_content = format!("/proc/{}/fdinfo/{}", pid, fd);
        let proc_fd = fs::read_to_string(file_content).ok()?;
        
        let file_cur = OpenFile::parse_cursor(&proc_fd)?;
        let file_acc_mod = OpenFile::parse_access_mode(&proc_fd)?;

        Some(OpenFile::new(proc_fd_name, file_cur, file_acc_mod))
