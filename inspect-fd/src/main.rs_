...
fn main() {
    ...
    let retparent = ps_utils::get_target(target).expect("error on getting target pid");

    match retparent {
        Some(ret) => 
        {
            ret.print(target);
            let retchild = ps_utils::get_child_processes(ret.pid).ok().unwrap();
            if !retchild.is_empty() {
                println!("\n{} child info: ", target);
            }

            {
                for child in retchild {
                    child.print(target);
                }
            }
        },
        ...
        None => 
        {
            eprintln!("Target {} did not match any running PIDs or executable.", target);
            std::process::exit(1);
        },
    }
